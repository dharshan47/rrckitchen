...
export function SupportClient() {
  ...
  const { data: searchResults, isLoading: isSearching } = useQuery({
    queryKey: ["supportSearch", debouncedSearch],
    queryFn: async () => {
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {["Login Issues", "Order Issues", "Payment Failed", "Delivery Delay", "OTP Not Received"].map(tag => (
                <button 
                  ...
                  className="px-3 py-1.5 text-xs font-medium bg-white border border-[#E5E7EB] rounded-full text-[#334155] hover:bg-[#F3FAF5] hover:border-[#CDEBD8] hover:text-[#087A36] transition-colors shadow-sm"
                >
                  {tag}
                </button>
              ))}
          ...
          <section className="min-h-[400px]">
             <div className="flex items-center justify-between mb-6">
                ...
                {!isSearching && searchResults && (
                  <span className="text-sm font-medium text-[#475569]">{searchResults.length} results found</span>
                )}
             </div>

             {isSearching ? (
                <div className="flex flex-col items-center justify-center py-12 space-y-4">
                  <div className="h-8 w-8 rounded-full border-4 border-[#087A36] border-t-transparent animate-spin" />
                  <p className="text-[#475569] font-medium text-sm">Searching topics...</p>
                </div>
                ...
                <div className="text-center py-12 bg-white rounded-[16px] border border-[#E5E7EB]">
                  <div className="inline-flex items-center justify-center h-16 w-16 rounded-full bg-[#F8FAFC] mb-4">
                    <Search className="h-8 w-8 text-[#94A3B8]" />
                  </div>
                  <h4 className="text-lg font-bold text-[#0F172A] mb-2">No results found</h4>
                  <p className="text-[#475569] text-sm max-w-sm mx-auto">
                    We couldn&apos;t find any topics matching &quot;{debouncedSearch}&quot;. Try using different keywords.
                  </p>
                </div>
             )}
          </section>
