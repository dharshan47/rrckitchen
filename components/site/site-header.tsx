...
export function SiteHeader() {
  ...
  return (
    <>
      {/* ── Home Page Navbar (transparent initially, sticky after hero) ── */}
      <header
        className={cn(
          "top-0 z-50 w-full transition-all duration-300",
          isHomePage
            ? isScrolled
              ? "absolute md:fixed bg-background border-b border-border shadow-sm animate-in slide-in-from-top-2"
        ...
        <div className="hidden md:block">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 h-16">
            ...
            <div className="flex items-center gap-6">
              <Link href="/search" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group">
                <Search className="h-5 w-5" />
                <span>Search</span>
              </Link>
              <Link href="/help" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group">
                <HelpCircle className="h-5 w-5" />
                <span>Help</span>
              </Link>
              {isLoggedIn ? (
                <Link href="/account/profile" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group" aria-label="Profile">
                  <User className="h-5 w-5" />
                  <span>Profile</span>
                </Link>
              ) : (
                <Link href="/login" className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group" aria-label="Login">
                  <User className="h-5 w-5" />
                  <span>Login</span>
                </Link>
              )}
              <Link href={cartHref} className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group relative" aria-label="Cart">
                <ShoppingCart className="h-5 w-5" />
                <span>Cart</span>
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-2 h-4 min-w-4 flex items-center justify-center rounded-full bg-primary text-primary-foreground p-0 text-[10px] font-bold">
                    {cartCount}
                  </span>
                )}
              </Link>
        ...
        {!hideNav && (
          <div className="md:hidden px-4 py-3">
            <div className="flex items-center justify-between">
              <button onClick={() => setLocationOpen(true)} className="flex items-center gap-2 text-sm font-medium text-foreground">
              ...
              <div className="flex items-center gap-2">
                {isLoggedIn ? (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button aria-label="User menu" className="h-8 w-8 flex items-center justify-center rounded-full border border-border text-foreground hover:bg-muted transition-colors">
                        <User className="h-5 w-5" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="min-w-40">
        ...
        {!hideNav && (
          <div className="md:hidden px-4 pb-3">
            <SearchAutocomplete
              ...
              inputClassName="h-10 rounded-lg text-sm pl-10 focus-visible:ring-1 bg-muted text-foreground placeholder:text-muted-foreground border border-border"
            />
          </div>
        )}
      </header>

      {/* ── Hero Carousel (directly attached below navbar, no border radius) ── */}
      ...
      <header className={`sticky top-0 z-50 border-b border-border bg-background mb-0 lg:mb-6 transition-opacity duration-300 ${
        isHomePage ? 'hidden' : 'hidden md:block'
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 h-20">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href={cartHref} className="flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors group relative" aria-label="Cart">
              ...
              <span>Cart</span>
              {cartCount > 0 && (
                <span className="absolute -top-1.5 -right-2 h-4 min-w-4 flex items-center justify-center rounded-full bg-primary p-0 text-[10px] font-bold text-primary-foreground">
                  {cartCount}
                </span>
              )}
            </Link>
