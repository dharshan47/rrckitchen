...
const KITCHEN_STATUS_BADGES: Record<
  string,
  { label: string; className: string }
> = {
  ACTIVE: {
    label: "Active",
    className: "bg-[#EFF8F2] text-[#087A35] border-transparent",
  },
  APPROVED: {
    label: "Approved",
    className: "bg-[#EFF6FF] text-[#2563EB] border-transparent",
  },
  PENDINGAPPROVAL: {
    label: "Pending Approval",
    className: "bg-[#FFF7E6] text-[#B45309] border-transparent",
  },
  SUSPENDED: {
    label: "Suspended",
    className: "bg-[#FEF2F2] text-[#C81E3A] border-transparent",
  },
  REJECTED: {
    label: "Rejected",
    className: "bg-[#FEF2F2] text-[#C81E3A] border-transparent",
...
export function EditMenuDashboard({ onClose }: EditMenuDashboardProps) {
  ...
  const kitchenStatusBadge = selectedKitchen
    ? KITCHEN_STATUS_BADGES[selectedKitchen.status]
    : null;
  ...
  const saveAll = async () => {
    ...
    for (const id of ids) {
      ...
      if (res.success) {
        ...
        toast.error(
          res.error ||
            `Failed to save ${items?.find((i) => i.id === id)?.name ?? "item"}`,
        );
      }
    }
  ...
  const handleSetMainPhoto = (index: number) => {
    ...
    updateDraft(selectedItemId, {
      photos: photos.map((p, idx) => ({ ...p, sortOrder: idx })),
    });
    setPreviewIndex(0);
  };
  ...
  const updateHighlight = (index: number, patch: Partial<EditorHighlight>) => {
    if (!selectedItemId || !draft) return;
    const highlights = draft.highlights.map((h, i) =>
      i === index ? { ...h, ...patch } : h,
    );
  ...
  const removeHighlight = (index: number) => {
    if (!selectedItemId || !draft) return;
    updateDraft(selectedItemId, {
      highlights: draft.highlights.filter((_, i) => i !== index),
    });
  };
  ...
  const toggleRelated = (id: string) => {
    ...
    updateDraft(selectedItemId, {
      relatedItemIds: current.includes(id)
  ...
  return (
    <div className="bg-[#FFFFFF] min-h-screen flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-[#FFFFFF] border-b border-[#EDF0F3] px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="hidden md:block">
            <h1 className="text-[24px] font-bold text-[#111827] leading-[32px] tracking-tight">
              Menu Detail Management
            </h1>
            <p className="text-[13px] text-[#64748B] mt-0.5">
              Manage and update menu details of your kitchens
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {kitchenSlug && (
            <Link href={`/kitchens/${kitchenSlug}`} target="_blank">
              <Button
                variant="outline"
                className="hidden sm:flex text-[#087A35] border-[#9CCDAE] bg-[#FFFFFF] hover:bg-[#F2FAF5] font-semibold h-[40px] shadow-[0_1px_2px_rgba(15,23,42,0.025)] rounded-[7px] px-4"
              >
          ...
          >
            <Trash2
              className="h-[16px] w-[16px] mr-2 text-[#475569]"
              strokeWidth={1.8}
            />
            Discard Changes
          </Button>
          ...
          >
            {saveMutation.isPending ? (
              <Loader2
                className="h-[16px] w-[16px] mr-2 animate-spin"
                strokeWidth={1.8}
              />
            ) : (
          ...
          <Button
            ...
            size="icon"
            onClick={onClose}
            className="ml-2 rounded-full h-[40px] w-[40px] bg-slate-100 hover:bg-slate-200 text-[#475569]"
          >
      ...
      <div className="bg-[#FFFFFF] border-b border-[#EDF0F3] px-4 sm:px-6 lg:px-8 py-3 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-[0_1px_2px_rgba(15,23,42,0.025)] z-20 relative">
        <div className="flex items-center gap-6 overflow-x-auto scrollbar-hide pb-2 md:pb-0">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {kitchenStatusBadge ? (
                    <>
                      <span
                        className={`h-2 w-2 rounded-full mr-1.5 ${kitchenStatusBadge.className.includes("green") || kitchenStatusBadge.className.includes("EFF8F2") ? "bg-[#087A35]" : kitchenStatusBadge.className.includes("red") || kitchenStatusBadge.className.includes("FEF2F2") ? "bg-[#DC2626]" : kitchenStatusBadge.className.includes("amber") || kitchenStatusBadge.className.includes("FFF7E6") ? "bg-[#F59E0B]" : "bg-[#2563EB]"}`}
                      />
          ...
          <Separator
            ...
            className="h-[42px] hidden md:block bg-[#EDF0F3]"
          />
          <div className="flex items-center gap-8 shrink-0 px-2">
            <div className="flex items-center gap-3">
              <UtensilsCrossed
                className="h-[24px] w-[24px] text-[#FF4B16]"
                strokeWidth={1.8}
              />
              <div>
                ...
                <p className="font-bold text-[18px] leading-none mt-1 text-[#111827]">
                  {items?.length ?? "—"}
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="relative w-full sm:w-64 hidden lg:block">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 h-[16px] w-[16px] text-[#475569]"
              strokeWidth={1.8}
            />
            <Input
              placeholder="Search menu items..."
              className="pl-9 h-[40px] border-[#DCE3EA] rounded-[7px] text-[14px] text-[#172033] placeholder:text-[#94A3B8] focus-visible:ring-0 focus-visible:border-[#087A35] shadow-none"
              value={searchTerm}
          ...
          <Button
            className="h-[40px] bg-[#087A35] hover:bg-[#06652C] text-[#FFFFFF] font-medium px-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] rounded-[7px]"
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          className={`w-full lg:w-[340px] flex-shrink-0 border-r border-[#E5EAF0] bg-[#FFFFFF] flex-col ${!selectedItemId ? "flex" : "hidden lg:flex"}`}
        >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4
                        className={`font-semibold text-[14px] truncate flex items-center gap-1.5 ${isSelected ? "text-[#087A35]" : "text-[#172033]"}`}
                      >
                        {item.name}
                        {isDirty && (
                          <span
                            className="h-1.5 w-1.5 rounded-full bg-[#FF4B16] shrink-0"
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        <div className="flex items-center gap-1.5">
                          <div
                            className={`h-[6px] w-[6px] rounded-full ${item.isAvailable ? "bg-[#087A35]" : "bg-[#FF4B16]"}`}
                          />
                          <span
                            className={`text-[12px] font-medium ${item.isAvailable ? "text-[#087A35]" : "text-[#FF4B16]"}`}
                          >
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      {item.avgRating > 0 ? (
                        ...
                        <span className="text-[13px] text-[#64748B] mr-2">
                          -
                        </span>
                      )}
                    </div>
        ...
        <div
          className={`flex-1 overflow-hidden flex flex-col bg-[#FAFBFC] ${selectedItemId ? "block" : "hidden lg:block"}`}
        >
                  ...
                  {isLoading ? (
                    ...
                    <>
                      ...
                      <p className="text-[14px] text-[#64748B]">
                        Choose an item from the list on the left, or create a
                        new one.
                      </p>
                      <Button
                        className="mt-2 h-10 bg-green-800 hover:bg-green-700 font-semibold"
                        onClick={() => setAddDialogOpen(true)}
                      >
              ...
              <div className="p-4 sm:p-6 lg:p-8 pb-0 shrink-0 bg-[#FAFBFC] z-10 border-b border-[#EDF0F3]">
                <div className="max-w-[1200px] mx-auto flex flex-col gap-5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <button
                        className="lg:hidden text-[#172033] mr-1"
                      ...
                      {draft.isAvailable ? (
                        <Badge className="bg-[#EFF8F2] text-[#087A35] hover:bg-[#EFF8F2] border-none px-2.5 py-0.5 rounded-full font-medium">
                          <div className="h-1.5 w-1.5 rounded-full bg-[#087A35] mr-1.5" />{" "}
                          Active
                        </Badge>
                      ) : (
                        <Badge className="bg-[#FEF2F2] text-[#DC2626] hover:bg-[#FEF2F2] border-none px-2.5 py-0.5 rounded-full font-medium">
                          <div className="h-1.5 w-1.5 rounded-full bg-[#DC2626] mr-1.5" />{" "}
                          Inactive
                        </Badge>
                      )}
                      {dirtyIds.includes(selectedItemId) && (
                        <Badge className="bg-[#FFF1EB] text-[#FF4B16] hover:bg-[#FFF1EB] border-none px-2.5 py-0.5 rounded-full font-medium">
                          Unsaved
                        </Badge>
                      )}
                    </div>
                    <div className="text-[13px] font-medium text-[#64748B] flex items-center gap-2">
                      ...
                      <span className="text-[#172033] font-bold">
                        {selectedRow?.publicCode ||
                          `MI-${selectedItemId.slice(0, 4).toUpperCase()}`}
                      </span>
                  ...
                  <ScrollArea className="w-full">
                    ...
                    <ScrollBar
                      ...
                      className="h-1.5 hidden sm:flex"
                    />
                  </ScrollArea>
                </div>
              </div>
                          ...
                          <CardHeader className="p-5 border-b border-[#EDF0F3] flex flex-row items-center justify-between">
                            ...
                            <CloudinaryUpload onUpload={handleUpload}>
                              {({ uploading, startUpload }) => (
                                <Button
                                  variant="outline"
                                  className="h-[36px] px-3 text-[13px] font-semibold border-[#DCE3EA] bg-[#FFFFFF] text-[#172033] hover:bg-[#F8FAFC] rounded-[7px] shadow-none"
                                  onClick={startUpload}
                                  disabled={uploading}
                                >
                                  {uploading ? (
                            ...
                            {draft.photos.length > 0 ? (
                              <>
                                <div className="relative aspect-video rounded-[8px] bg-slate-100 overflow-hidden mb-4 border border-[#E5EAF0]">
                                  ...
                                  />
                                  {draft.bestseller && (
                                    <Badge className="absolute top-4 left-4 bg-[#FF4B16] hover:bg-[#FF4B16] text-[#FFFFFF] border-none shadow-none rounded-[6px] px-3 py-1 font-bold">
                                      Bestseller
                                    </Badge>
                                  )}
                                  <button
                                ...
                                <p className="text-[12px] text-[#64748B] mt-2 font-medium">
                                  Recommended: 1200x900px or higher. First image
                                  is the main one — double-click a thumbnail to
                                  change it.
                                </p>
                              </>
                            ) : (
                              <div className="border-[2px] border-dashed border-[#DCE3EA] bg-[#F8FAFC] rounded-[8px] py-16 flex flex-col items-center gap-3 text-[#94A3B8]">
                                <ImageIcon
                                  className="h-10 w-10 text-[#CBD5E1]"
                                  strokeWidth={1.5}
                                />
                                <p className="text-[14px] font-medium text-[#64748B]">
                                  No images yet — upload photos for this dish
                                </p>
                              </div>
                            )}
                          </CardContent>
                      ...
                      <div className="space-y-8">
                          ...
                          <CardContent className="p-5 space-y-5">
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                ...
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[12px] text-[#64748B] font-medium">
                            ...
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                ...
                                />
                                <span className="absolute bottom-3 right-3 text-[12px] text-[#64748B] font-medium">
                            ...
                            <div className="flex items-center justify-between p-4 border border-[#E5EAF0] rounded-[7px] bg-[#FFFFFF]">
                              ...
                              <Switch
                                ...
                                onCheckedChange={(v) =>
                                  updateDraft(selectedItemId, { bestseller: v })
                                }
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                          {draft.cuisine && (
                            <Badge
                              variant="outline"
                              className="bg-[#EFF8F2] text-[#087A35] border-[#9CCDAE] py-1.5 px-3 font-medium shadow-none rounded-[6px]"
                            >
                              <UtensilsCrossed
                          ...
                          <Badge
                            variant="outline"
                            className={`${draft.foodType === "NONVEG" ? "bg-[#FEF2F2] text-[#DC2626] border-[#FCA5A5]" : "bg-[#EFF8F2] text-[#087A35] border-[#9CCDAE]"} py-1.5 px-3 font-medium shadow-none rounded-[6px]`}
                          >
                            <span
                              className={`h-3 w-3 rounded-sm border ${draft.foodType === "NONVEG" ? "border-[#DC2626] bg-[#FEF2F2]" : "border-[#087A35] bg-[#EFF8F2]"} flex items-center justify-center mr-1.5 text-[8px]`}
                            >
                              <div
                                className={`h-1.5 w-1.5 rounded-full ${draft.foodType === "NONVEG" ? "bg-[#DC2626]" : "bg-[#087A35]"}`}
                              />
                            </span>
                          ...
                          {draft.bestseller && (
                            <Badge
                              variant="outline"
                              className="bg-[#FFF1EB] text-[#FF4B16] border-[#FF4B16]/30 py-1.5 px-3 font-medium shadow-none rounded-[6px]"
                            >
                              <Star
                                className="h-3.5 w-3.5 mr-1.5"
                                strokeWidth={1.8}
                          ...
                          <CardContent className="p-5 space-y-2">
                            <div className="flex items-center justify-between py-2 border-b border-[#EDF0F3]">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  updateDraft(selectedItemId, {
                                    serves: numOrNull(e.target.value),
                                  })
                                }
                                placeholder="Persons"
                              />
                            ...
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateDraft(selectedItemId, {
                                      aboutTitle: e.target.value.slice(0, 100),
                                    })
                                  }
                                ...
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-[#64748B] font-medium">
                                  {draft.aboutTitle.length}/100
                                </span>
                              </div>
                            </div>
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                <Textarea
                                  ...
                                  onChange={(e) =>
                                    updateDraft(selectedItemId, {
                                      aboutDescription: e.target.value.slice(
                                        0,
                                        500,
                                      ),
                                    })
                                ...
                                />
                                <span className="absolute bottom-3 right-3 text-[11px] text-[#64748B] font-medium">
                                  {draft.aboutDescription.length}/500
                                </span>
                              </div>
                            </div>
                          </CardContent>
                          ...
                          <Card className="shadow-none border-[#E5EAF0] bg-[#FFFFFF] rounded-[9px]">
                            ...
                            <CardContent className="p-5 space-y-5">
                              <div className="grid grid-cols-2 gap-5">
                                <div className="grid gap-2">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      updateDraft(selectedItemId, {
                                        price: numOrNull(e.target.value) ?? 0,
                                      })
                                    }
                              ...
                              {draft.compareAtPrice &&
                                draft.compareAtPrice > draft.price && (
                              ...
                              <div className="flex items-center justify-between p-4 border border-[#E5EAF0] rounded-[7px] bg-[#FFFFFF]">
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  />
                                  <span
                                    className={`text-[13px] font-bold ${draft.isAvailable ? "text-[#087A35]" : "text-[#DC2626]"}`}
                                  >
                                    {draft.isAvailable
                                      ? "Available"
                                      : "Unavailable"}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                              ...
                              <div className="grid gap-2">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateDraft(selectedItemId, {
                                      deliveryFee: numOrNull(e.target.value),
                                    })
                                  }
                                  placeholder="e.g. 49"
                                  className="h-[40px] text-[14px] font-medium text-[#111827] border-[#DCE3EA] rounded-[7px] shadow-none focus-visible:ring-0 focus-visible:border-[#087A35]"
                      ...
                      <div className="space-y-8">
                          ...
                          <CardContent className="p-5 space-y-5">
                              ...
                              <div className="grid gap-2">
                                ...
                                <div className="flex items-center gap-2 border border-[#DCE3EA] rounded-[7px] px-3 h-[40px] bg-[#F8FAFC]">
                                  ...
                                  <span className="font-bold text-[14px] text-[#111827]">
                                    {selectedRow?.avgRating
                                      ? Number(selectedRow.avgRating).toFixed(1)
                                      : "—"}
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="flex items-center gap-2 bg-[#EFF8F2] text-[#087A35] p-3 rounded-[7px] border border-[#9CCDAE]">
                              ...
                              <span className="text-[12px] font-bold">
                                Ratings & orders update automatically
                              </span>
                            </div>
                          </CardContent>
                        </Card>
                          ...
                          <CardContent className="p-5 space-y-4">
                            {draft.highlights.length === 0 && (
                              <p className="text-[13px] font-medium text-[#64748B]">
                                No highlights yet — add the reasons customers
                                will love this dish.
                              </p>
                            )}
                            {draft.highlights.map((h, i) => (
                              <div
                                key={i}
                                className="border border-[#E5EAF0] rounded-[7px] p-4 space-y-3 bg-[#F8FAFC]/50"
                              >
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateHighlight(i, {
                                      description: e.target.value.slice(0, 60),
                                    })
                                  }
                            ...
                            <Button
                              variant="outline"
                              className="w-full h-[40px] border-dashed border-[#087A35]/50 text-[#087A35] hover:bg-[#EFF8F2] hover:text-[#087A35] font-bold bg-[#FFFFFF] rounded-[7px] shadow-none"
                        ...
                        <Card className="shadow-none border-[#E5EAF0] bg-[#FFFFFF] rounded-[9px]">
                          <CardHeader className="p-5 border-b border-[#EDF0F3] flex flex-row items-center justify-between">
                            ...
                            <span className="text-[12px] font-medium text-[#64748B]">
                              ({draft.relatedItemIds.length} items)
                            </span>
                          </CardHeader>
                          <CardContent className="p-5">
                            <div className="max-h-64 overflow-y-auto space-y-3 custom-scrollbar pr-2">
                              {(options?.menuItems ?? [])
                                ...
                                .map((m) => {
                                  const checked = draft.relatedItemIds.includes(
                                    m.id,
                                  );
                                  return (
                                    <label
                                    ...
                                    >
                                      <div className="flex items-center gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                          <span
                                            className={`font-bold text-[13px] truncate block ${checked ? "text-[#087A35]" : "text-[#111827]"}`}
                                          >
                                            {m.name}
                                          </span>
                                      ...
                                      {!m.isAvailable && (
                                        <Badge
                                          variant="outline"
                                          className="h-[20px] px-1.5 py-0 text-[10px] text-[#94A3B8] border-[#DCE3EA] shrink-0 font-bold rounded-[4px]"
                                        >
                                          Hidden
                                        </Badge>
                                      )}
                          ...
                          <CardContent className="p-5 space-y-5">
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                ...
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-[#94A3B8] font-medium">
                            ...
                            <div className="grid gap-2">
                              ...
                              <div className="relative">
                                <Textarea
                                  ...
                                  onChange={(e) =>
                                    updateDraft(selectedItemId, {
                                      metaDescription: e.target.value.slice(
                                        0,
                                        160,
                                      ),
                                    })
                                ...
                                />
                                <span className="absolute bottom-2 right-3 text-[11px] text-[#94A3B8] font-medium">
        ...
        <DialogContent className="sm:max-w-md rounded-[9px] border-[#E5EAF0]">
          ...
          <div className="space-y-5 py-2">
            <div className="space-y-2.5">
              <Label
                htmlFor="editor-item-name"
                className="text-[13px] font-bold text-[#172033]"
              >
                Item Name
              </Label>
              <Input
          ...
          <DialogFooter>
            <Button
              variant="outline"
              className="h-[40px] text-[14px] font-bold rounded-[7px] border-[#DCE3EA] text-[#172033] hover:bg-[#F8FAFC]"
