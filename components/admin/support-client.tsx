...
const statusConfig: Record<string, { label: string; className: string }> = {
  OPEN: {
    label: "OPEN",
    className:
      "text-[#2563EB] bg-[#EFF6FF] border border-[#BFDBFE] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  INPROGRESS: {
    label: "IN PROGRESS",
    className:
      "text-[#EA580C] bg-[#FFF7ED] border border-[#FED7AA] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  RESOLVED: {
    label: "RESOLVED",
    className:
      "text-[#15803D] bg-[#F0FDF4] border border-[#BBF7D0] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  CLOSED: {
    label: "CLOSED",
    className:
      "text-[#334155] bg-[#F1F5F9] border border-[#E2E8F0] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
};
...
const priorityConfig: Record<string, { label: string; className: string }> = {
  LOW: {
    label: "LOW",
    className:
      "text-[#475569] bg-[#F1F5F9] border border-[#E2E8F0] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  MEDIUM: {
    label: "MEDIUM",
    className:
      "text-[#F97316] bg-[#FFF7ED] border border-[#FED7AA] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  HIGH: {
    label: "HIGH",
    className:
      "text-[#EF4444] bg-[#FFF1F1] border border-[#FFD2D2] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
  URGENT: {
    label: "URGENT",
    className:
      "text-[#EF3340] bg-[#FFF1F1] border border-[#FFD2D2] font-bold text-[10px] px-2.5 py-1 rounded-[5px]",
  },
};

const categoryConfig: Record<
  string,
  { icon: LucideIcon; label: string; description: string; barColor: string }
> = {
  delivery: {
    icon: Truck,
    label: "Delivery Issue",
    description:
      "Related to delivery time, partner behavior, location or tracking issues.",
    barColor: "bg-[#7C3AED]",
  },
  payment: {
    icon: CreditCard,
...
function formatTicketId(id: string, publicCode?: string | null) {
  return (
    publicCode ??
    `TKT-${format(new Date(), "yyyy")}-${id.slice(-4).toUpperCase()}`
  );
}
...
export default function AdminSupportPage() {
  ...
  const thisWeek = useMemo(
    () =>
      tickets.filter((t) => new Date(t.createdAt).getTime() >= weekStart)
        .length,
    [tickets, weekStart],
  );
    ...
    const withAdminReply = tickets
      .filter(
        (t) =>
          ...
          t.messages.some((m) => m.senderId !== t.userId),
      )
      .map((t) => {
  ...
  const columns = useMemo(
    () => [
        ...
        cell: (info) => {
          ...
          return (
            <div className="flex items-center gap-3 relative">
              <div
                className={cn(
                  "absolute -left-4 w-[2px] h-[28px] rounded-r-md",
                  catInfo.barColor,
                )}
              />
      ...
      columnHelper.accessor("category", {
        ...
        cell: (info) => {
          const catInfo =
            categoryConfig[info.getValue()] || categoryConfig.other;
        ...
        <div className="px-4 md:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 text-[#334155] ${isFetching ? "animate-spin" : ""}`}
                  strokeWidth={1.8}
                />{" "}
                Refresh
              </Button>
              ...
              {[
                {
                  ...
                  icon: Ticket,
                  iconBg: "bg-[#EFF8F1]",
                  iconColor: "text-[#3E9645]",
                  border: "border-[#D9EDDB]",
                  changeUp: true,
                },
                {
                  label: "Open Tickets",
                ...
                >
                  <div
                    className={cn(
                      "h-[44px] w-[44px] rounded-full flex items-center justify-center flex-shrink-0 border",
                      card.border,
                      card.iconBg,
                    )}
                  >
                    <card.icon
                      className={cn("h-[22px] w-[22px]", card.iconColor)}
                      strokeWidth={2}
                    />
                  </div>
                  <div>
                    ...
                    <p className="text-[22px] font-bold text-[#111827] leading-tight mt-1">
                      {card.value}
                    </p>
                    {card.change ? (
                  ...
                  {[
                    {
                      ...
                      colors:
                        activeTab === "ALL"
                          ? "bg-[#F2FAF3] text-[#15803D] border-[#A7DDAE]"
                          : "bg-transparent text-[#64748B] border-transparent",
                      countBg:
                        activeTab === "ALL"
                          ? "bg-[#DCF2DF] text-[#15803D]"
                    ...
                    },
                    {
                      ...
                      colors:
                        activeTab === "OPEN"
                          ? "bg-[#FFF8F1] text-[#F97316] border-[#FDE3BF]"
                          : "bg-transparent text-[#64748B] border-transparent",
                      countBg:
                        activeTab === "OPEN"
                          ? "bg-[#FFF0DD] text-[#EA580C]"
                          : "bg-[#F1F5F9] text-[#64748B]",
                    },
                    {
                      key: "INPROGRESS",
                      ...
                      colors:
                        activeTab === "INPROGRESS"
                          ? "bg-[#F3F7FF] text-[#2563EB] border-[#D8E7FF]"
                          : "bg-transparent text-[#64748B] border-transparent",
                      countBg:
                        activeTab === "INPROGRESS"
                    ...
                    },
                    {
                      ...
                      colors:
                        activeTab === "RESOLVED"
                          ? "bg-[#F2FAF3] text-[#16A34A] border-[#A7DDAE]"
                          : "bg-transparent text-[#64748B] border-transparent",
                      countBg:
                        activeTab === "RESOLVED"
                          ? "bg-[#DCF2DF] text-[#16A34A]"
                          : "bg-[#F1F5F9] text-[#64748B]",
                    },
                    {
                      key: "CLOSED",
                      ...
                      colors:
                        activeTab === "CLOSED"
                          ? "bg-[#FFFFFF] text-[#334155] border-[#E2E8F0]"
                          : "bg-transparent text-[#64748B] border-transparent",
                      countBg:
                        activeTab === "CLOSED"
                    ...
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2 px-3.5 py-2 rounded-[8px] border text-[13px] font-bold transition-all whitespace-nowrap",
                        tab.colors,
                      )}
                    >
                      {tab.label}
                      <span
                        className={cn(
                          "rounded-[5px] px-1.5 py-0.5 text-[10px] font-extrabold",
                          tab.countBg,
                        )}
                      >
                        {tab.count}
          ...
          {isError && (
            <div className="flex items-center gap-2.5 bg-[#FEF2F2] border border-[#FECACA] text-[#DC2626] text-[13px] font-semibold rounded-[10px] px-4 py-3">
              <AlertCircle className="h-4 w-4 shrink-0" /> Failed to load
              tickets.
              <button
                onClick={() => refetch()}
            ...
            <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-[10px] shadow-none w-full">
                ...
                <Table className="w-full text-sm min-w-[800px]">
                  <TableHeader>
                    {table.getHeaderGroups().map((headerGroup) => (
                      <TableRow
                        key={headerGroup.id}
                        className="border-b border-[#EEF0F2] hover:bg-[#FFFFFF]"
                      >
                    ...
                    {table.getRowModel().rows?.length ? (
                      ...
                      <TableRow>
                        <TableCell
                          ...
                          className="h-24 text-center py-16 text-[#94A3B8] text-sm font-medium border-b-0"
                        >
                          No tickets found
                        </TableCell>
                      </TableRow>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-5 py-4 border-t border-[#EEF0F2] bg-[#FFFFFF] gap-4">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      return (
                        <Button
                          ...
                          className={cn(
                            ...
                            currentPage === page
                              ? "bg-[#3E9645] text-[#FFFFFF] border-[#3E9645]"
                              : "bg-[#FFFFFF] text-[#334155] border-[#E2E8F0] hover:bg-[#F0F8F1] hover:text-[#15803D]",
                          )}
                        >
                          {page}
                        </Button>
                  ...
                  {totalPages > 5 && (
                    <>
                      <span className="text-[12px] text-[#94A3B8] px-2 hidden sm:inline">
                        ...
                      </span>
                      <Button
                        variant="ghost"
                  ...
                  <div className="ml-4 flex items-center gap-2">
                    <span className="text-[12px] text-[#334155] font-medium hidden sm:inline">
                      Rows per page
                    </span>
                    <Select
      ...
      >
        ...
        >
          {selectedTicket && (
            <>
              {/* Fixed Header */}
              <div className="flex items-center justify-between px-6 py-5 flex-shrink-0 border-b border-[#EEF0F2]">
                <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-full hover:bg-[#FAFAFB] flex items-center justify-center text-[#334155] hover:text-[#111827] transition-colors"
                >
                    ...
                    return (
                      <div className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-[10px] p-4 flex items-start gap-4 mb-6">
                        <div className="h-[44px] w-[44px] rounded-[12px] bg-[#EFF8F1] flex items-center justify-center flex-shrink-0">
                          <CatIcon
                            className="h-[22px] w-[22px] text-[#16A34A]"
                            strokeWidth={1.8}
                          />
                        </div>
                        ...
                        >
                          <Avatar className="h-[32px] w-[32px]">
                            {!isAdmin && selectedTicket.user?.image ? (
                              ...
                              <AvatarFallback
                                className={cn(
                                  "text-[11px] font-bold border border-[#E5E7EB]",
                                  isAdmin
                                    ? "bg-[#F0F8F1] text-[#15803D]"
                ...
                <div className="px-6 py-5 space-y-4">
                    {selectedTicket.order ? (
                        ...
                        <div className="rounded-[10px] border border-[#E5E7EB] p-4 flex flex-col gap-3">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[13px] font-bold text-[#16A34A] font-mono">
                              {selectedTicket.order.publicCode
                                ? `ORD-${selectedTicket.order.publicCode}`
                                : `ORD-${selectedTicket.order.id.slice(-8).toUpperCase()}`}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-semibold text-[#475569]">
                            ...
                            <span className="text-[13px] font-bold text-[#111827]">
                              ₹
                              {Number(
                                selectedTicket.order.totalAmount,
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[12px] font-semibold text-[#334155]">
                              {format(
                                new Date(selectedTicket.order.createdAt),
                                "dd MMM yyyy, hh:mm a",
                              )}
                            </span>
                          </div>
                        ...
                        <div className="rounded-[10px] border border-[#E5E7EB] p-4 flex flex-col gap-3">
                          ...
                          {selectedTicket.order.orderItems.length > 0 ? (
                            selectedTicket.order.orderItems.map((item) => (
                              ...
                              >
                                <span className="text-[13px] font-medium text-[#334155] flex-1">
                                  {item.menuItem.name}
                                </span>
                      ...
                      <div className="flex flex-col items-center justify-center h-full gap-3 text-center py-10">
                        ...
                        <p className="text-[12px] font-medium text-[#94A3B8] max-w-[240px]">
                          This ticket was raised without a related order.
                        </p>
                      </div>
                    )}
                </div>
                      ...
                      <div>
                        ...
                        <p className="text-[12px] font-medium text-[#64748B] mt-0.5">
                          ...
                          {selectedTicket.userId
                            ? selectedTicket.userId.slice(-8).toUpperCase()
                            : "Unknown"}
                        </p>
                      </div>
                    </div>
                    <div className="rounded-[10px] border border-[#E5E7EB] p-4 flex flex-col gap-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[13px] font-bold text-[#334155] break-all">
                            {selectedTicket.user?.email || "—"}
                          </p>
                        </div>
                      </div>
              ...
              {/* Reply Box */}
              {detailTab === "conversation" && (
                <div className="border-t border-[#EEF0F2] px-6 py-5 bg-[#FFFFFF]">
                  <p className="text-[12px] font-bold text-[#111827] mb-3">
                    Reply to customer
                  </p>
                  <Textarea
                    {...register("message")}
