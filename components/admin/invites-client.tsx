...
import type { AdminInviteRow, ActiveAdminRow } from "@/stores/adminInvitesStore"

const PERMISSION_CONFIG: { key: AdminPermission; label: string; icon: typeof UserCheck }[] = [
...
const permConfigOf = (key: AdminPermission) =>
  PERMISSION_CONFIG.find((p) => p.key === key) ?? { key, label: key, icon: ShieldCheck }
...
type InviteStatus = "ACTIVE" | "EXPIRING_SOON" | "EXPIRING_TODAY" | "USED" | "REVOKED" | "EXPIRED"
...
function computeAvgJoinDays(invites: AdminInviteRow[]) {
  ...
  const totalMs = usedInvites.reduce(
    (acc, i) => acc + (new Date(i.consumedAt!).getTime() - new Date(i.createdAt).getTime()),
    0
  )
...
function PermissionBadges({ permissions, size = "sm" }: { permissions: AdminPermission[]; size?: "sm" | "md" }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      {visible.map((p) => {
        ...
        return (
          <div key={p} title={permConfigOf(p).label} className={`${size === "sm" ? "h-7 w-7" : "h-6 w-6"} rounded bg-gray-100 border border-gray-200 flex items-center justify-center`}>
            <Icon className={`${size === "sm" ? "h-3.5 w-3.5" : "h-3 w-3"} text-gray-600`} />
          </div>
        )
      })}
      {rest > 0 && (
        <div className={`${size === "sm" ? "h-7 w-7" : "h-6 w-6"} rounded bg-gray-50 border border-gray-200 flex items-center justify-center text-[10px] font-medium text-gray-600`}>
          +{rest}
        </div>
      )}
    </div>
...
function InviteAvatar({ label, className }: { label: string; className: string }) {
  return (
    <div className={`${className} rounded-full flex items-center justify-center font-bold text-sm shrink-0`}>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-4">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm h-[120px] flex flex-col justify-between">
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col">
      ...
      <div className="flex-1 p-0">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center gap-4 p-4 pl-6 border-b border-gray-50">
            <Skeleton className="h-10 w-10 rounded-full" />
...
export default function AdminInvitesClient() {
  ...
  const [inviteToDelete, setInviteToDelete] = useState<AdminInviteRow | null>(null)
  ...
  const [adminToManage, setAdminToManage] = useState<ActiveAdminRow | null>(null)
  ...
  const toggleAllPermissions = () => {
    setGeneratedLink(null)
    setSelectedPermissions((prev) =>
      prev.length === PERMISSION_CONFIG.length ? [] : PERMISSION_CONFIG.map((p) => p.key)
    )
  }
  ...
  const stats = useMemo(() => {
    ...
    const newThisMonth = admins.filter((a) => a.status === "ACTIVE" && isNewThisMonth(a.joinedOn)).length
  ...
  const filteredAdmins = useMemo(() => {
    ...
    return admins.filter((a) => {
      ...
      if (adminPermissionFilter !== "all" && !a.permissions.includes(adminPermissionFilter as AdminPermission)) return false
      return true
    })
  ...
  const pagedAdmins = filteredAdmins.slice(safeAdminPage * adminPageSize, safeAdminPage * adminPageSize + adminPageSize)

  const exportAdminsCSV = () => {
    const header = ["Name", "Email", "Permissions", "Invited By", "Joined On", "Last Active", "Status"]
  ...
  const handleManageAdmin = (admin: ActiveAdminRow) => {
    const action = admin.status === "ACTIVE" ? deactivateMutation : reactivateMutation
    action.mutateAsync(admin.id).then((res) => {
      if (res.ok) {
        toast.success(admin.status === "ACTIVE" ? "Admin deactivated" : "Admin reactivated")
    ...
    <div className="min-h-screen bg-[#F8F9FB] p-4 md:p-6 lg:p-8 font-sans">
      <div className="max-w-[1440px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl md:text-[28px] font-bold text-gray-900 tracking-tight">Admin Invites & Access Management</h1>
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-100">
                <ShieldCheck className="h-5 w-5 text-violet-600" />
              </div>
            </div>
            <p className="text-gray-500 mt-1 text-sm md:text-base">Invite new administrators and manage their access permissions</p>
          </div>
          <a
            href="#create-invite"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm relative overflow-hidden flex flex-col justify-between h-[120px]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500">Permissions Used</p>
                  <h3 className="text-2xl font-bold text-gray-900 leading-tight">{stats.usedPermCount} / {PERMISSION_CONFIG.length}</h3>
                </div>
              </div>
              <div className="mt-2 flex flex-col gap-1.5 w-full">
                <span className="text-[11px] font-medium text-gray-500">{stats.permPct}% of available</span>
                <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-green-500 rounded-full" style={{ width: `${stats.permPct}%` }} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm relative overflow-hidden flex flex-col justify-between h-[120px]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500">Total Invites Sent</p>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm relative overflow-hidden flex flex-col justify-between h-[120px]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500">Avg. Join Time</p>
                  <h3 className="text-2xl font-bold text-gray-900 leading-tight">
                    {stats.avgJoinDays === null ? "—" : stats.avgJoinDays >= 1 ? `${stats.avgJoinDays.toFixed(1)} days` : `${Math.round(stats.avgJoinDays * 24)} hours`}
                  </h3>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-2 gap-6">
          {isLoading ? (
            ...
            <div id="create-invite" className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col scroll-mt-6">
              <div className="p-6 border-b border-gray-100">
                <h2 className="text-lg font-bold text-gray-900">Create New Admin Invite</h2>
                <p className="text-sm text-gray-500 mt-1">Select permissions for the admin you want to invite</p>
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-gray-900">Select Permissions</h3>
                  <button
                  ...
                  >
                    {selectedPermissions.length === PERMISSION_CONFIG.length ? "Deselect All" : "Select All"}
                  </button>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center justify-center h-full gap-2 py-2">
                          <Icon className={`h-5 w-5 ${selected ? "text-violet-600" : "text-gray-400 group-hover:text-gray-500"}`} />
                          <span className={`text-[11px] font-medium text-center ${selected ? "text-violet-900" : "text-gray-600"}`}>{label}</span>
                        </div>
                      </button>
                    )
                  })}
                ...
                <div className="mt-auto">
                  <h3 className="text-sm font-bold text-gray-900">Invite Link Preview</h3>
                  <p className="text-xs text-gray-500 mt-1 mb-4">A secure, one-time link will be generated</p>
                  ...
                  {generatedLink ? (
                    <div className="flex flex-col gap-3 mb-4">
                      ...
                      <p className="text-[11px] font-medium text-gray-500">Link expires in 48 hours or after first use.</p>
                    </div>
                  ) : (
                    <div className="bg-violet-50 text-violet-700 text-xs font-medium px-4 py-3 rounded-lg flex items-center gap-2 w-full mb-4 border border-violet-100">
                      ...
                      This invite link will expire in 48 hours or after first use.
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      disabled={createInviteMutation.isPending || selectedPermissions.length === 0}
                    >
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col">
              <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                ...
                <span className="text-xs font-medium text-gray-500">{invites.length} total</span>
              </div>
              <ScrollArea className="p-0 flex-1 w-full">
                {invites.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Generate your first invite link to get started</p>
                  </div>
                ) : (
                  <table className="w-full text-left min-w-[640px]">
                    <tbody>
                      {invites.map((invite) => {
                        ...
                        const isDone = status === "USED" || status === "REVOKED" || status === "EXPIRED"
                        return (
                          <tr key={invite.id} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${isDone ? "opacity-70" : ""}`}>
                            <td className="p-4 pl-6">
                              <div className="flex items-center gap-4">
                                <InviteAvatar label={invite.id} className={`h-10 w-10 ${isDone ? "bg-gray-100 text-gray-500" : "bg-violet-100 text-violet-700"}`} />
                                <div>
                                  <p className={`text-sm font-bold text-gray-900 ${status === "REVOKED" ? "line-through text-gray-400" : ""}`}>
                                    Invite #{invite.id.slice(0, 4).toUpperCase()}-{invite.id.slice(-4).toUpperCase()}
                                  </p>
                                  <p className="text-xs text-gray-500 mt-0.5">Created by {invite.createdBy ?? "Unknown"}</p>
                                  <p className="text-xs text-gray-400 mt-0.5">
                                    {formatDate(invite.createdAt)}
                                    {status === "USED" && invite.consumedAt ? ` • Used on ${formatDate(invite.consumedAt)}` : status === "REVOKED" ? " • Revoked" : status === "EXPIRED" ? " • Expired" : ` • Expires in ${daysUntil(invite.expiresAt)} day${daysUntil(invite.expiresAt) !== 1 ? "s" : ""}`}
                                  </p>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="p-4">
                              <span className={`px-3 py-1 border rounded-full text-xs font-semibold whitespace-nowrap ${statusStyle[status]}`}>{statusLabel[status]}</span>
                            </td>
                            <td className="p-4 pr-6">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <button
                                  ...
                                  onClick={() => copyText(buildInviteUrl(invite.token), "Invite link")}
                                  title="Copy link"
                                >
                                  <Copy className="h-4 w-4" />
                                </button>
                                <button
                                  ...
                                  title={isDone ? "Delete invite" : "Revoke invite"}
                                >
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex-1 overflow-hidden flex flex-col">
              <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-lg font-bold text-gray-900 whitespace-nowrap">Active Admins <span className="text-gray-400 font-normal">({admins.length})</span></h2>
              ...
              <ScrollArea className="w-full">
                {filteredAdmins.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-sm font-bold text-gray-500">No admins found</p>
                  ...
                  <table className="w-full text-left text-sm min-w-[900px]">
                    <thead className="bg-white border-b border-gray-100">
                      <tr>
                        <th className="py-3 px-6 font-semibold text-gray-900">Admin</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Contact</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Permissions</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Invited By</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Joined On</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Last Active</th>
                        <th className="py-3 px-4 font-semibold text-gray-900">Status</th>
                        <th className="py-3 px-6 font-semibold text-gray-900 text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {pagedAdmins.map((admin) => (
                        <tr key={admin.id} className="hover:bg-gray-50/50 transition-colors group">
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              {admin.image ? (
                                ...
                                <img src={admin.image} alt={admin.name} className="h-8 w-8 rounded-full border border-gray-200 object-cover" />
                              ) : (
                                <InviteAvatar label={admin.name} className="h-8 w-8 bg-violet-100 text-violet-700" />
                              )}
                              <div className="flex items-center gap-2">
                                <span className="font-bold text-gray-900">{admin.name}</span>
                                {admin.userId === currentUserId && (
                                  <span className="bg-violet-100 text-violet-700 text-[10px] font-bold px-2 py-0.5 rounded-full">You</span>
                                )}
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-4 text-gray-600">{admin.invitedBy ?? "—"}</td>
                          <td className="py-4 px-4 text-gray-600">{formatDate(admin.joinedOn)}</td>
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              <div className={`h-1.5 w-1.5 rounded-full ${admin.lastActive ? "bg-green-500" : "bg-gray-300"}`}></div>
                              <span className="text-gray-900 font-medium">{timeAgo(admin.lastActive)}</span>
                            </div>
                          </td>
                          <td className="py-4 px-4">
                            <span className={`px-3 py-1 border rounded-full text-xs font-semibold ${admin.status === "ACTIVE" ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}>
              ...
              <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm bg-gray-50/30">
                <span className="text-gray-500">
                  Showing {filteredAdmins.length === 0 ? 0 : safeAdminPage * adminPageSize + 1} to {Math.min((safeAdminPage + 1) * adminPageSize, filteredAdmins.length)} of {filteredAdmins.length} admins
                </span>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-1">
                    ...
                    {Array.from({ length: adminTotalPages }).slice(0, 5).map((_, i) => (
                      <button
                        key={i}
                        className={`h-8 w-8 flex items-center justify-center rounded-lg font-medium transition-colors ${
                          i === safeAdminPage ? "bg-violet-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100"
                        }`}
                    ...
                    {adminTotalPages > 5 && <span className="text-[13px] font-bold text-gray-500 px-1">...</span>}
                    <button
                      className="h-8 w-8 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <div className="w-full 2xl:w-[320px] shrink-0 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-gray-900">Security Tips</h3>
              </div>
              <div className="space-y-4">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    <div className="text-sm font-bold text-gray-900">Share invite links securely</div>
                    <div className="text-xs text-gray-500 mt-0.5">Anyone with the link can join</div>
                  </div>
                </div>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    <div className="text-sm font-bold text-gray-900">Review permissions regularly</div>
                    <div className="text-xs text-gray-500 mt-0.5">Ensure least privilege access</div>
                  </div>
                </div>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    <div className="text-sm font-bold text-gray-900">Revoke unused invites</div>
                    <div className="text-xs text-gray-500 mt-0.5">Keep your system secure</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Dialog open={!!inviteToView} onOpenChange={(open) => !open && setInviteToView(null)}>
        <DialogContent className="sm:max-w-[460px]">
          <DialogHeader>
            <DialogTitle>Invite Details</DialogTitle>
            <DialogDescription>
              {inviteToView ? `Invite #${inviteToView.id.slice(0, 4).toUpperCase()}-${inviteToView.id.slice(-4).toUpperCase()} · ${statusLabel[getInviteStatus(inviteToView)]}` : ""}
            </DialogDescription>
          </DialogHeader>
          {inviteToView && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50/60 p-4">
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-bold text-gray-500">Invite Link</p>
                  <p className="text-[13px] font-semibold text-gray-900 truncate mt-0.5">{buildInviteUrl(inviteToView.token)}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  className="shrink-0 gap-1.5 text-[12px] font-bold"
                  onClick={() => copyText(buildInviteUrl(inviteToView.token), "Invite link")}
                  disabled={getInviteStatus(inviteToView) !== "ACTIVE" && getInviteStatus(inviteToView) !== "EXPIRING_SOON" && getInviteStatus(inviteToView) !== "EXPIRING_TODAY"}
                >
              ...
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">Created By</p>
                  <p className="font-bold text-gray-900 mt-1 truncate">{inviteToView.createdBy ?? "Unknown"}</p>
                </div>
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">Created At</p>
                  <p className="font-bold text-gray-900 mt-1">{formatDateTime(inviteToView.createdAt)}</p>
                </div>
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">Expires At</p>
                  <p className="font-bold text-gray-900 mt-1">{formatDateTime(inviteToView.expiresAt)}</p>
                </div>
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">{inviteToView.consumedAt ? "Used At" : inviteToView.revokedAt ? "Revoked At" : "Status"}</p>
                  <p className="font-bold text-gray-900 mt-1">
                    {inviteToView.consumedAt ? formatDateTime(inviteToView.consumedAt) : inviteToView.revokedAt ? formatDateTime(inviteToView.revokedAt) : statusLabel[getInviteStatus(inviteToView)]}
                  </p>
                </div>
              </div>
              <div>
                <p className="text-[11px] font-bold text-gray-500 mb-2">Permissions ({inviteToView.permissions.length})</p>
                <div className="flex flex-wrap gap-2">
                  {inviteToView.permissions.map((p) => (
                    <Badge key={p} variant="outline" className="bg-violet-50/60 text-violet-700 border-violet-200 font-bold text-[11px] px-2.5 py-1 rounded-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInviteToView(null)}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!inviteToDelete} onOpenChange={(open) => !open && setInviteToDelete(null)}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>
              {inviteToDelete && (getInviteStatus(inviteToDelete) === "USED" || getInviteStatus(inviteToDelete) === "REVOKED" || getInviteStatus(inviteToDelete) === "EXPIRED")
                ? "Delete invite?"
                : "Revoke invite?"}
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInviteToDelete(null)}>Cancel</Button>
            <Button
              variant="destructive"
              disabled={deleteMutation.isPending || revokeMutation.isPending}
              onClick={() => {
                if (!inviteToDelete) return
                const done = getInviteStatus(inviteToDelete) === "USED" || getInviteStatus(inviteToDelete) === "REVOKED" || getInviteStatus(inviteToDelete) === "EXPIRED"
                if (done) handleDeleteInvite()
                else handleRevokeInvite(inviteToDelete)
              }}
            >
              {deleteMutation.isPending || revokeMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
              {inviteToDelete && (getInviteStatus(inviteToDelete) === "USED" || getInviteStatus(inviteToDelete) === "REVOKED" || getInviteStatus(inviteToDelete) === "EXPIRED") ? "Delete" : "Revoke"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* Admin manage dialog */}
      <Dialog open={!!adminToManage} onOpenChange={(open) => !open && setAdminToManage(null)}>
        <DialogContent className="sm:max-w-[440px]">
          ...
          {adminToManage && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50/60 p-4">
                {adminToManage.image ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={adminToManage.image} alt={adminToManage.name} className="h-11 w-11 rounded-full border border-gray-200 object-cover" />
                ) : (
                  <InviteAvatar label={adminToManage.name} className="h-11 w-11 bg-violet-100 text-violet-700" />
                )}
                <div className="min-w-0">
                  <p className="text-sm font-extrabold text-gray-900 truncate">{adminToManage.name}</p>
                  <p className="text-xs font-medium text-gray-500 truncate">{adminToManage.email || "No email"}</p>
                </div>
                <Badge className={`ml-auto ${adminToManage.status === "ACTIVE" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600"} border-0 font-bold`}>
                  {adminToManage.status === "ACTIVE" ? "Active" : "Inactive"}
                </Badge>
              </div>
              <div>
                <p className="text-[11px] font-bold text-gray-500 mb-2">Permissions ({adminToManage.permissions.length})</p>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-[13px]">
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">Invited By</p>
                  <p className="font-bold text-gray-900 mt-1 truncate">{adminToManage.invitedBy ?? "—"}</p>
                </div>
                <div className="rounded-xl border border-gray-100 p-3.5">
                  <p className="text-[11px] font-bold text-gray-500">Joined On</p>
                  <p className="font-bold text-gray-900 mt-1">{formatDate(adminToManage.joinedOn)}</p>
                </div>
                <div className="rounded-xl border border-gray-100 p-3.5 col-span-2">
                  <p className="text-[11px] font-bold text-gray-500">Last Active</p>
                  <p className="font-bold text-gray-900 mt-1">{timeAgo(adminToManage.lastActive)}</p>
                </div>
              </div>
            </div>
          )}
          <DialogFooter className="sm:justify-between">
            ...
            {adminToManage && adminToManage.userId !== currentUserId && (
              <Button
                variant={adminToManage.status === "ACTIVE" ? "destructive" : "default"}
              ...
              >
                ...
                {adminToManage.status === "ACTIVE" ? "Deactivate Admin" : "Reactivate Admin"}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
